...
parser.add_argument("--data_path", type=str, required=True, help="Test JSON dataset (e.g., ANES)")
...
def get_vote_probs(messages, max_new_tokens=10):
    ...
    for candidate in CANDIDATES:
        ...
        with torch.no_grad():
            for token_id in candidate_ids:
                logits = model(current_input_ids).logits[:, -1, :]
    ...
    return {c: (p / total if total > 0 else 1/len(CANDIDATES)) for c,p in candidate_probs.items()}
...
if ft_data:
    ...
    # Prepare text for causal LM: prompt + target
    ft_texts = []
    for item in ft_data:
        prompt_text = "\n".join(f"{m['role']}: {m['content']}" for m in item.get("messages", []))
        target_text = next((m['content'] for m in item.get("messages", []) if m['role']=="assistant"), "")
        ft_texts.append({"text": prompt_text + tokenizer.eos_token + target_text + tokenizer.eos_token})
