...
# Arguments
# -----------------------------
parser = argparse.ArgumentParser(description="Fine-tune LLMs on OpinionQA/Subpop")
...
if args.fine_tune_data is not None:
    ...
    if args.fine_tune_data.endswith(".jsonl"):
        ft_data = [json.loads(line) for line in open(args.fine_tune_data)]
    else:
...
model = AutoModelForCausalLM.from_pretrained(
    args.model_name,
    device_map="auto",  # auto assigns multi-GPU if available
...
def get_vote_probs(messages, max_new_tokens=10):
    """Deterministic candidate probabilities"""
        ...
        with torch.no_grad():
            for token_id in candidate_ids:
                ...
                current_input_ids = torch.cat(
                    [current_input_ids, torch.tensor([[token_id]]).to(device)], dim=1
                )
...
if ft_data is not None:
    ...
    training_args = TrainingArguments(
        ...
        logging_steps=50,
        save_strategy="no",
        fp16=False,  # <-- FP32 safe
        seed=args.seed,
        report_to=[],  # disables WandB prompt
    )
...
print("Inference completed, total samples:", len(df_final))
