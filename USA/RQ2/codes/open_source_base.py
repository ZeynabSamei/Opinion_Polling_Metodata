...
if args.election_year == 2020:
    CANDIDATES = ["Donald Trump", "Joe Biden", "Other"]
...
# Mapping votes to numeric indices
...
def normalize_vote(text):
    """Map text to one of the candidates, or 'Other'."""
    if text is None:
        return "Other"
    t = text.lower()
    for c in CANDIDATES:
...
def get_vote_probs(messages):
    """Compute probability distribution over all candidates."""
    ...
    with torch.no_grad():
        for candidate in CANDIDATES:
            ...
            for token_id in cand_ids:
                ...
                curr_ids = torch.cat([curr_ids, torch.tensor([[token_id]], device=device)], dim=1)
...
# -----------------------------
# Compute Wasserstein distance for each sample
# -----------------------------
def sample_wasserstein(probs, ground_truth):
    """Compare predicted distribution vs one-hot ground truth."""
    gt_onehot = np.zeros(len(CANDIDATES))
    gt_onehot[VOTE2IDX[ground_truth]] = 1.0
    pred_probs = np.array([probs[c] for c in CANDIDATES])
    return wasserstein_distance(gt_onehot, pred_probs)

df_final["wasserstein"] = df_final.apply(lambda row: sample_wasserstein(row["probs"], row["ground_truth"]), axis=1)
