...
QUESTIONS = [
    ...
    {"col": "vote_choice",
     "question": "Which presidential candidate did you vote for in the 2024 election? 'Kamala Harris', 'Donald Trump', or 'someone else'?",
...
SYSTEM_PROMPT = (
    ...
    "Answer the interview questions truthfully based on the information provided. "
    "Just choose from the list of options with no more explanations."
)

# ==========================================
# 4. Build Interviews (MESSAGES FORMAT)
# ==========================================
chat_data = []
...
for _, row in tqdm(df.iterrows(), total=len(df)):
    for HR_OMIT in [q["col"] for q in QUESTIONS]:
        ...
        for q in QUESTIONS:
            ...
            if q["vals"] is None:
                user_text += f"- Respondent: {int(row[col])}\n\n"
            else:
                val = q["vals"].get(int(row[col]))
                if val is not None:
                    user_text += f"- Respondent: {val}\n\n"

        # Add omitted question (no answer)
        omit_q = next(q for q in QUESTIONS if q["col"] == HR_OMIT)
        user_text += f"- {omit_q['question']}\n"

        # Ground-truth answer
        if omit_q["vals"] is None:
            assistant_text = str(int(row[HR_OMIT]))
        else:
            assistant_text = omit_q["vals"].get(int(row[HR_OMIT]), "unknown")
...
print(f"Saved dataset with messages format: {output_file}")
