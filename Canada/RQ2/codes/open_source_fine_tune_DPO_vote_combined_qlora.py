...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Unified QLoRA training (SFT + DPO) for immigration preference prediction."
    )
    parser.add_argument(
        "--model_name",
        type=str,
    ...
    parser.add_argument(
        "--sft_files",
        nargs="+",
        default=["dataset_ft/tweets_ft_immigration.jsonl"],
    ...
    parser.add_argument(
        "--dpo_files",
        nargs="+",
        default=["dataset_ft/tweets_ft_immigration_dpo.jsonl"],
    ...
    parser.add_argument("--out_dir", type=str, default="./results_immigration")
    parser.add_argument("--tmp_dir", type=str, default="./tmp_qlora_immigration")
...
def build_dpo_dataset(
    ...
    for item in ft_rows:
        ...
        # Auto-generate DPO pairs from SFT data
            ...
            # Generate rejected by picking a random other immigration preference
...
def evaluate(
    ...
    df.to_csv(result_base + "_results_immigration_ft_dpo.csv", index=False)
    with open(result_base + "_metrics_immigration_ft_dpo.json", "w", encoding="utf-8") as f:
...
def train_dpo(
    ...
    # Check TRL version for correct DPOTrainer API
...
def main() -> None:
    ...
    if args.stage in ["dpo", "sft_then_dpo"]:
        ...
        if ref_model_name != args.model_name:
            ...
            # Wrap ref_model with PEFT using "default" adapter
    ...
    # Create PEFT model with "default" adapter name
