...
SYSTEM_PROMPT = (
    ...
    "Reason carefully about the demographic characteristics, political preferences, "
    ...
    "Rules:\n"
    "- The value of predict must be exactly one of: Liberal, Conservative, Other.\n"
    "- The reason should explain the main demographic and political factors considered.\n"
    "- Do not include markdown.\n"
    "- Do not include any text outside the JSON object.\n\n"

    "Definition:\n"
    "Other includes New Democratic Party (NDP), Bloc Québécois, Green Party, "
...
for model_name in MODELS:
    ...
    for start in tqdm(
        ...
        idxs, prompts, user_texts, gts = zip(
            *batch
        )
        ...
        for idx, user_text, gt, output in zip(
            ...
            pred, reason = parse_reasoning_output(
                output
            )
            results.append(
                {
    ...
    out_path = os.path.join(
        OUT_DIR,
        f"{model_tag}_results_party_reasoning.csv"
    )
    df.to_csv(
        out_path,
        index=False
