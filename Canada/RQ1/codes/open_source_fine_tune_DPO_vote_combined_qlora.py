...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Unified QLoRA training (SFT + DPO) for vote choice prediction."
    )
    parser.add_argument(
        "--model_name",
        type=str,
    ...
    parser.add_argument(
        "--sft_files",
        nargs="+",
        default=["dataset_ft/agg_ft_vote_2020.jsonl"],
    ...
    parser.add_argument(
        "--dpo_files",
        nargs="+",
        default=["dataset_ft/agg_ft_vote_2020_dpo.jsonl"],
    ...
    parser.add_argument("--out_dir", type=str, default="./results_vote")
    parser.add_argument("--tmp_dir", type=str, default="./tmp_qlora_vote")
...
def build_dpo_dataset(
    ...
    for item in ft_rows:
        ...
        # Auto-generate DPO pairs from SFT data
            ...
            # Generate rejected by picking a random other candidate
...
def evaluate(
    ...
    df.to_csv(result_base + "_results_vote_ft_dpo.csv", index=False)
    with open(result_base + "_metrics_vote_ft_dpo.json", "w", encoding="utf-8") as f:
