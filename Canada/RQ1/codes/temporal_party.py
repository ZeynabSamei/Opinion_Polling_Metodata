...
TASK_CONFIGS = {
    "party_choice": {
        ...
        "system_text": (
            "You are an expert political analyst specializing in Canadian federal elections "
            "and voting behavior.\n\n"
            "Your task is to predict the respondent's ideological self-placement based on the "
            "demographic and background information provided. Output strictly one label: "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Temporal Generalization Experiments with Canada party-choice survey data"
    )
    parser.add_argument("--model_name", type=str, default="meta-llama/Llama-3.1-8B-Instruct")
    parser.add_argument("--task", type=str, default="party_choice", choices=["party_choice"])
    ...
    parser.add_argument("--out_dir", type=str, default="./results_temporal_canada_party")
    parser.add_argument("--tmp_dir", type=str, default="./tmp_qlora_temporal_canada_party")
    ...
    # NEW: last cumulative/sequential "anchor" test year. Defaults to the most recent year in YEARS.
...
def experiment_1_sequential(args, base_model, tokenizer, candidates, system_text, data_by_year, years):
    print("\n" + "=" * 70)
    print("EXPERIMENT 1: Sequential (Train on N, Test on next available year)")
    print("=" * 70)
    results, all_preds = [], []
