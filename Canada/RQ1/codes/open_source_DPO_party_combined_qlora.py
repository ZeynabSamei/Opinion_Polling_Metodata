...
def main() -> None:
    ...
    for i, ft_file in enumerate(args.ft_files):
        ...
        if model is None:
            ...
            # Subsequent datasets: drop the previous adapter's weights before
            # adding a fresh one, so adapters never stack or leak into each
            # other's "reference" (adapters-disabled) forward pass.
            prev_name = model.active_adapters[0] if hasattr(model, "active_adapters") else model.active_adapter
            model.delete_adapter(prev_name)
