...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="QLoRA DPO training + deterministic vote-choice evaluation."
    )
    parser.add_argument(
        "--model_name",
        type=str,
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[
            "dataset_ft/agg_ft_vote_2020_dpo.jsonl",
        ],
    )
    parser.add_argument("--out_dir", type=str, default="./results_dpo_vote")
    parser.add_argument("--tmp_dir", type=str, default="./tmp_qlora_dpo_vote")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--max_len", type=int, default=400)
    parser.add_argument("--max_prompt_len", type=int, default=350)
...
def build_dpo_dataset(
    ...
    for item in ft_rows:
        ...
        # Also support the original messages format for compatibility
            ...
            # Create a simple rejected example (other candidate)
...
def evaluate(
    ...
    df.to_csv(result_base + "_results_dpo_vote.csv", index=False)
    with open(result_base + "_metrics_dpo_vote.json", "w", encoding="utf-8") as f:
